...
PortViki=$(echo "$(tail -n 50 /linuxcash/cash/conf/drivers/hw::Serial_1.xml | grep "ttyS91" | wc -l)")
PortShtrih=$(echo "$(tail -n 50 /linuxcash/cash/conf/drivers/hw::GenericTcp_0.xml | grep "192.168.137.111" | wc -l)")
...
send_log ()
{
	...
	statusftpfile=$(curl -v -u ftpuser:GbplfnsqGfhjkm ftp://10.9.0.1/ftp/salerestrict/ | grep "${LOGFILEPATHONKASSA}" | wc -l )
		if [ $statusftpfile -eq 1 ]; then
echo `date +'d.%m.%Y %H:%M:%S'` "- Файл найден на сервере FTP!" >> $LOGFILEPATH
curl -v -u ftpuser:GbplfnsqGfhjkm ftp://10.9.0.1/ -Q "DELE /home/ftpuser/ftp/salerestrict/${LOGFILEPATHONKASSA}"
...
echo `date +'d.%m.%Y %H:%M:%S'` "- Файл на FTP-сервер Загружен!" >> $LOGFILEPATH
    curl --upload-file "$LOGFILEPATH" "ftp://$FTPUser:$FTPPassword@$FTPServer$FTPPath"
...
send_log_utm ()
{
	...
	statusftpfiles=$(curl -v -u ftpuser:GbplfnsqGfhjkm ftp://10.9.0.1/ftp/kassinfo/ | grep "${LOGFILEPATHONKASS}" | wc -l )
		if [ $statusftpfiles -eq 1 ]; then
echo `date +'d.%m.%Y %H:%M:%S'` "- Файл найден на сервере FTP!" >> $LOGFILEPAT
	curl -v -u ftpuser:GbplfnsqGfhjkm ftp://10.9.0.1/ -Q "DELE /home/ftpuser/ftp/kassinfo/${LOGFILEPATHONKASS}"
...
echo `date +'d.%m.%Y %H:%M:%S'` "- Файл на FTP-сервер Загружен!" >> $LOGFILEPAT
    curl --upload-file "$LOGFILEPAT" "ftp://$FTPUser:$FTPPassword@$FTPServer$FTPPaths"

}
#ТЕЛО СКРИПТА

#Первое условие. Если справочники не выгрузились, то вырубаем УТМ и ФР.
	if [[ ($check2 -eq 0 && $check -eq 0) && (($PortTransport -eq 1 && $PortAgent -eq 1) || (( -f /linuxcash/cash/conf/drivers/hw::Serial_1.xml && $PortViki -eq 1 ) || ( -f /linuxcash/cash/conf/drivers/hw::GenericTcp_0.xml && $PortShtrih -eq 1 ))) ]]; then
...
if [ -f /linuxcash/cash/conf/drivers/hw::Serial_1.xml ]; then  sed -i 's/ttyS91/ttyS92/g' /linuxcash/cash/conf/drivers/hw::Serial_1.xml
...
if [ -f /linuxcash/cash/conf/drivers/hw::GenericTcp_0.xml ]; then sed -i 's/192.168.137.111/192.168.138.111/g' /linuxcash/cash/conf/drivers/hw::GenericTcp_0.xml
...
#Второе условие. Если справочники выгрузились, то востанавливаем работу УТМ и ФР.
	if [[ ($check2 -ge 1 || $check -ge 1) && (($PortTransport -eq 0 && $PortAgent -eq 0) || (( -f /linuxcash/cash/conf/drivers/hw::Serial_1.xml && $PortViki -eq 0 ) || ( -f /linuxcash/cash/conf/drivers/hw::GenericTcp_0.xml && $PortShtrih -eq 0 ))) ]]; then
...
if [ -f /linuxcash/cash/conf/drivers/hw::Serial_1.xml ]; then  sed -i 's/ttyS92/ttyS91/g' /linuxcash/cash/conf/drivers/hw::Serial_1.xml
		echo `date +'%d.%m.%Y %H:%M:%S'` "Меняем символическую ссылку на вернe. для ВИКИ ПРИНТ" >> $LOGFILEPATH
		echo `date +'%d.%m.%Y %H:%M:%S'` "Перезапускаем кассувую программу" >> $LOGFILEPATH
			sudo pkill artix-gui
fi
if [ -f /linuxcash/cash/conf/drivers/hw::GenericTcp_0.xml ]; then sed -i 's/192.168.138.111/192.168.137.111/g' /linuxcash/cash/conf/drivers/hw::GenericTcp_0.xml
...
#Третье условие. Если справочники не выгрузились, но уже были применены ограничения по продажам(просто информативный лог)	
	if [[ ($check2 -eq 0 && $check -eq 0) && (($PortTransport -eq 0 && $PortAgent -eq 0) && (( -f /linuxcash/cash/conf/drivers/hw::Serial_1.xml && $PortViki -eq 0 ) || ( -f /linuxcash/cash/conf/drivers/hw::GenericTcp_0.xml && $PortShtrih -eq 0 ))) ]]; then
...
#Четвертое условие. Если справочники выгружены, то ничего не делать(просто информативный лог)		
	if [[ ($check2 -ge 1 || $check -ge 1) && (($PortTransport -eq 1 && $PortAgent -eq 1) && (( -f /linuxcash/cash/conf/drivers/hw::Serial_1.xml && $PortViki -eq 1 ) || ( -f /linuxcash/cash/conf/drivers/hw::GenericTcp_0.xml && $PortShtrih -eq 1 ))) ]]; then
...
sudo echo \*\/$Minute \* \* \* \* $InstallPath\/mrcutmstatus.sh $InstallPath >> /tmp/crontab.tmp
